...
import { DATA_REFRESHED_BUS_KEY, useFetch, type ApiCreateServiceReturn } from '@/shared/api'
...
export function useCreateContest(): UseCreateContestReturn {
  ...
  const saveCreatedId = (): number | null => {
    const id = Number(response.value?.data.id)

    return id || null
  }

  const createdId = computed<number | null>(saveCreatedId)
